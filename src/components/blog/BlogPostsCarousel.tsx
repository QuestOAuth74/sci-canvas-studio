      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        stopOnInteraction: true
          ...
          <CarouselContent className="-ml-4">
            {posts.map(post => <CarouselItem key={post.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <Card className="h-full cursor-pointer group hover:shadow-xl transition-all duration-300 overflow-hidden border-[3px] border-foreground neo-shadow hover:neo-shadow-xl" onClick={() => navigate(`/blog/${post.slug}`)}>
                  {post.featured_image_url && <div className="relative h-48 overflow-hidden">
                  ...
                  <CardContent className="p-6 space-y-4">
                    ...
                    {post.categories && post.categories.length > 0 && <div className="flex flex-wrap gap-2">
                        {post.categories.slice(0, 2).map(category => <Badge key={category.id} variant="secondary" className="font-semibold" style={{
                    ...
                    {/* Excerpt */}
                    {post.excerpt && <p className="text-sm text-muted-foreground line-clamp-2 font-medium">
                        {post.excerpt}
                      </p>}
